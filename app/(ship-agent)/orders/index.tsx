...
export default function ManagedOrdersList() {
  const router = useRouter();
  const userId = useAuthStore((s) => s.session?.user?.id ?? null);
  const { data: orders, isLoading, error, refetch } = useOrders('ship_agent', userId);

  return (
    <View style={styles.screen}>
      <Appbar.Header style={styles.appbar}>
        <Appbar.Content title="Managed Orders" titleStyle={styles.appbarTitle} />
      </Appbar.Header>
      ...
      {isLoading ? (
        ...
        <View style={styles.stateBox}>
          ...
          <Text style={styles.stateText}>Couldn't load managed orders.</Text>
        </View>
      ) : orders.length === 0 ? (
        <View style={styles.stateBox}>
          <Ionicons name="clipboard-outline" size={32} color={palette.hullGray} />
          <Text style={styles.stateText}>No vessels assigned to you yet.</Text>
        </View>
      ) : (
        <FlatList
          data={orders}
          ...
          onRefresh={refetch}
          renderItem={({ item }) => (
            <OrderRow order={item} onPress={() => router.push(`/(ship-agent)/orders/${item.id}`)} />
          )}
        />
      )}
    </View>
